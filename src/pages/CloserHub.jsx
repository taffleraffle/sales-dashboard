...
const WHAT = {
  retainer: 'The month-to-month Local SEO client agreement.',
  trial: 'The two-week Local Surge trial agreement.',
}
const FILLS = [['ClientName', 'company'], ['MonthlyFee', 'fee'], ['SpecialConditions', 'special conditions'], ['OptRepName', 'OPT signer name'], ['ExecutionDate', 'date']]
function TemplateNote({ id, templates, settings, region }) {
  ...
  return (
    <div className="grid gap-1" style={{ padding: '10px 12px', border: '1px solid var(--rule)', borderRadius: 10, background: 'var(--paper-2)', marginTop: 6 }}>
      ...
      {t?.fields && (() => {
        const fills = FILLS.filter(([f]) => t.fields.includes(f)).map(([, w]) => w)
        return <div style={line}><span style={k}>Fills in</span>{fills.length ? <span>{fills.join(', ')}, from this card.</span> : <span style={{ color: 'var(--house-warn)' }}>Nothing fills in automatically. Open the draft in PandaDoc and fill it in before you send it.</span>}</div>
      })()}
      ...
      {t && (
        <div style={line}><span style={k}>Who signs</span>
          {missing.includes('the client')
            ? <span style={{ color: 'var(--house-warn)' }}>No signature box for the client. The hub will not draft from this until one is added in the PandaDoc template editor.</span>
            : missing.includes('OPT')
              ? <span>The client signs. There is no OPT signature box on this one.</span>
              : <span>The client signs, then OPT countersigns.</span>}
        </div>
      )}
    </div>
  )
    ...
    <div className="grid gap-4">
      ...
      <div className="tile" style={{ padding: '22px 24px' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Contract" hint="Picked from the deal type and region. The note below says what it is.">
            <select value={form.template} onChange={set('template')}>
              <option value="">Pick one</option>
              {CONFIGURED.filter(([k]) => settings[k]).map(([k]) => {
                const t = templates.find(x => x.id === settings[k])
                return <option key={k} value={settings[k]}>{t ? t.name.trim() : settings[k]}</option>
              })}
