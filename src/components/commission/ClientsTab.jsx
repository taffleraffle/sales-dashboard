      ...
      <div className={`expand-section ${showAddClient ? 'max-h-[600px] opacity-100 mb-4' : 'max-h-0 opacity-0'}`}>
        <div className="tile tile-feedback p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            ...
            <div><label className="text-[11px] text-text-400 uppercase tracking-wider block mb-1">Trial Start</label><EditorialDate value={newClient.trial_start_date} onChange={(v) => setNewClient(c => ({ ...c, trial_start_date: v }))} fullWidth /></div>
            <div><label className="text-[11px] text-text-400 uppercase tracking-wider block mb-1">Ascension Date</label><EditorialDate value={newClient.ascension_date} onChange={(v) => setNewClient(c => ({ ...c, ascension_date: v }))} fullWidth /></div>
            ...
            <div><label className="text-[11px] text-text-400 uppercase tracking-wider block mb-1">Next Billing Date</label><EditorialDate value={newClient.next_billing_date || ''} onChange={(v) => setNewClient(c => ({ ...c, next_billing_date: v }))} fullWidth /></div>
