...
export default function SyncStatusIndicator({ pinned = ['meta', 'marketingTracker'] }) {
  ...
  const summaryLabel = summaryAge === null ? 'Not synced yet' : `Synced ${formatAge(summaryAge)}`
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(v => !v)}
        className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-bg-card border border-border-default text-xs text-text-secondary hover:border-opt-yellow/20 transition-colors min-h-[40px]"
        title="Data sync status"
      >
        <Clock size={ICON.sm} className="text-text-400" />
        <span className="hidden sm:inline">{summaryLabel}</span>
        <span className="sm:hidden">{summaryAge === null ? 'never' : formatAge(summaryAge)}</span>
      </button>
      ...
      {open && (
        <div className="absolute top-full right-0 mt-2 dropdown-panel min-w-[300px]">
          ...
          <div className="py-1">
            {statuses.map(s => (
              <div key={s.key} className="flex items-center justify-between gap-3 px-4 py-2 text-[12px]">
