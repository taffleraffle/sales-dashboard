    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="tile tile-feedback w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div>
            ...
            <textarea value={form.summary} onChange={e => set('summary', e.target.value)} rows={8}
              className="w-full bg-bg-primary border border-border-default rounded-sm px-3 py-2 text-sm text-text-primary outline-none focus:border-opt-yellow/40 resize-y"
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-border-default">
          <button onClick={onClose} className="px-4 py-2 text-xs text-text-secondary border border-border-default rounded-sm hover:bg-bg-card-hover">Cancel</button>
          <button onClick={handleSave} disabled={saving}
            className="px-4 py-2 text-xs bg-opt-yellow text-text-primary rounded-sm font-medium hover:brightness-110 disabled:opacity-50 flex items-center gap-1.5">
