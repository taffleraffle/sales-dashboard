-- 067_lib_ad_performance_status.sql
--
-- Surface Meta ad status on lib_ad_performance so the Creatives library can
-- filter "Live only" vs "Paused" vs "All" without a second round-trip.
--
-- New columns appended to RETURNS TABLE:
--   status            TEXT  - the raw ad.status field from Meta (ACTIVE, PAUSED, ARCHIVED, DELETED, etc.)
--   effective_status  TEXT  - the rollup status accounting for campaign/adset state
--   is_live           BOOL  - effective_status = 'ACTIVE' (convenience for filtering)
...
LANGUAGE SQL STABLE AS $$
  ...
  SELECT
    ...
    a.status,
    a.effective_status,
    (a.effective_status = 'ACTIVE') AS is_live,
