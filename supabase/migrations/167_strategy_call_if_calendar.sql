-- 167: add the "Strategy Call - IF" Calendly mirror to the strategy-calendar allowlist
-- (Will 2026-08-17).
--
-- BUG: on 2026-08-16 the Typeforms were repointed from the `Opt Digital | Strategy
-- Call` Calendly link to `Opt Digital | Strategy Call - IF`. That link mirrors into
-- its OWN GHL calendar, WnXnhT0m3MNezzoFQbcG ("Opt Digital | Strategy Call - IF
-- (Calendly)"), which was never added to the allowlist. `lib_strategy_booking_resolved`
-- INNER JOINs ghl_appointments against `strategy_calendars`, so every booking taken
-- since the cutover was silently dropped: zero bookings counted from 2026-08-16 on.
--
-- This is the third repeat of the same failure (2026-05-14 woLoGz, 2026-07-16 el8rJ /
-- migration 158). Same fix: add a NULL-audience-hint UNION ALL entry so the
-- funnel-first resolver decides audience per prospect, then refresh the matviews.
--
-- View body is migration 163's, unchanged apart from the new UNION ALL branch.
...
-- Backfill so the post-cutover bookings appear immediately.
